...
def check_collision(obj1, obj2):
    ...
    if (abs_dx*abs_dx + abs_dy*abs_dy) <= (both*both):
        ...
        # Reduced mass
        # u = 1/((1.0/obj1.mass) + (1.0/obj2.mass))
        ...
        if((obj2.vel-obj1.vel).dot(n_hat) > 0):
            ...
            J = ((1+e) * u * (obj2.vel-obj1.vel).dot(n_hat)) * n_hat
        
        # Update each object's momentum with the calculated impulse
        obj1.mom += J
        obj2.mom -= J
...
"Stops the circles from spinning when e < 1"
"Keep angular momentum constant by decreasing the kinetic energy and keeping it the same"

"L = mr x v"
"L = ur x deltaV = u * r * nHat x deltaV"
"L = Lexp + (r+d) * nHat x J"
"((1 - ((r+d)/r))*L) = J (perpendicular)"
"((r - (r+d)) / (r * (r+d))) * L"
"((-d) / (r * (r+d))) * L = J (perpendicular)"

"r x deltaV = r.cross(vel - diff)"
"nHat = n.perpendicular()"

"""
J only happens when they're traveling towards each other
Jperp happens all the time
"""
